...
q_linebase = bb_tokyo %>% 
  opq(timeout = 60) %>% 
  add_osm_feature("route", "train|subway")
...
q_keihintohoku = q_linebase %>% add_osm_feature("ref", "JK") %>% add_osm_feature("from:en", "Omiya")

# TokyoMetro
q_ginza     = q_linebase_subway %>% add_osm_feature("name:en", "Tokyo Metro Ginza Line (Shibuya --> Asakusa)")
q_marunouchi= q_linebase_subway %>% add_osm_feature("name:en", "Marunouchi Line (Ogikubo --> Ikebukuro)")
q_hibiya    = q_linebase_subway %>% add_osm_feature("name:en", "Tokyo Metro Hibiya Line (Kita-Senju --> Naka-meguro)")
q_tozai     = q_linebase_subway %>% add_osm_feature("ref", "T") %>% add_osm_feature("from", "中野")
q_chiyoda   = q_linebase_subway %>% add_osm_feature("name:en", "Tokyo Metro Chiyoda Line (Ayase --> Yoyogi-Uehara)")
q_yurakucho = q_linebase_subway %>% add_osm_feature("ref", "Y") %>% add_osm_feature("from", "新木場")
q_hanzomon  = q_linebase_subway %>% add_osm_feature("ref", "Z") %>% add_osm_feature("to", "押上")
q_namboku   = q_linebase_subway %>% add_osm_feature("name:en", "Tokyo Metro Namboku Line (Akabane-iwabuchi --> Meguro)")
q_fukutoshin= q_linebase_subway %>% add_osm_feature("name:en", "Tokyo Metro Fukutoshin Line (Shibuya --> Wakoshi)")

# Toei
q_asakusa   = q_linebase_subway %>% add_osm_feature("name:en", "Toei Asakusa Line (Oshiage --> Nishi-magome)")
q_mita      = q_linebase_subway %>% add_osm_feature("name:en", "Toei Mita Line (Nishi-takashimadaira --> Meguro)")
q_shinjuku  = q_linebase_subway %>% add_osm_feature("name:en", "Shinjuku Line (Shinjuku --> Motoyawata)")
q_oedo      = q_linebase_subway %>% add_osm_feature("name:en", "Oedo Line (Hikarigaoka --> Tochomae)")

# retrieval
retrieve_line_sf = function(query) {
  osmdata_sf(query)$osm_lines %>% st_transform(6677) %>% select(osm_id, name.en)
}
...
lines_sf = rbind(yamanote_sf, chuo_sf, keihintohoku_sf,
                 ...
                 chiyoda_sf, yurakucho_sf, hanzomon_sf, namboku_sf, fukutoshin_sf,
  ...
  mutate(col = case_when(name.en == "Yamanote Line"               ~ "#80c241",
                         ...
                         name.en == "Tokyo Metro Ginza Line"      ~ "#f7931d",
                         name.en == "Tokyo Metro Marunouchi Line" ~ "#ed1c24",
                         name.en == "Tokyo Metro Hibiya Line"     ~ "#8ba2ae",
                         name.en == "Tokyo Metro Tōzai Line"      ~ "#00b2dd",
                         name.en == "Tokyo Metro Chiyoda Line"    ~ "#00a650",
                         name.en == "Tokyo Metro Yūrakuchō Line"  ~ "#bb8b38",
                         name.en == "Tokyo Metro Hanzōmon Line"   ~ "#937cb9",
                         name.en == "Tokyo Metro Namboku Line"    ~ "#00b5ad",
                         name.en == "Tokyo Metro Fukutoshin Line" ~ "#ba6831",
                         name.en == "Toei Asakusa Line"           ~ "#ef5ba1",
                         name.en == "Toei Mita Line"              ~ "#0080c6",
                         name.en == "Toei Shinjuku Line"          ~ "#6dc067",
                         name.en == "Toei Oedo Line"              ~ "#b51e82"))
...
getFareFromLegs = function(legs) {
  ...
  for(i in 1:length(legs)) {
    ...
    if(leg$mode == "WALK" & i == length(legs) & i != 1) {
      # if i is 1 then it means the trip is done purely by walking
      # and we shouldn't do anything; fare defaults to 0
...
```{r Investigate routes, eval=F, include=F}
