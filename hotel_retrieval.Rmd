...
grid_routes %>% 
  ...
  select(fromID, toID, option, duration, distance, fare, transfers)
...
grid_routes_all_vars %>% 
  select(fromID, toID, duration,
         option, walkTime, transitTime, waitingTime, walkDistance, transfers,
         leg, leg.from.stopId, leg.to.stopId, leg.mode, leg.distance) %>% 
  mutate_at(vars(leg.from.stopId, leg.to.stopId, leg.mode, leg.distance), unlist)
...
grid_aggr_speed = grid_routes %>% 
  ...
  mutate(speed = (walkDistance + distance)/(duration/60)) %>% 
  group_by(fromPlace) %>% 
