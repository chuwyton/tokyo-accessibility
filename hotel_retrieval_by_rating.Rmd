...
get_topn_avg_ols = function(gr, n) {
  # gr_ = gr %>%
  #   group_by(fromID) %>%
  ...
  gr_ = gr %>% 
    ...
    summarise_at(c("duration", "fare", "transfers"),
                 list(avg = ~.x[order(duration)[1:n]] %>% mean())) %>% 
  ...
  ols = lm(review.average ~ price.lead.average * star.average +
             duration.avg + fare.avg + transfers.avg,
           data = h_aggr)
  return(ols)
}

hotels_aggr_all_ols = map(1:17, ~get_topn_avg_ols(grid_routes_all_vars, .x))
...
hotels_sf_access = grid_routes_all_vars %>% 
  group_by(fromID) %>% 
  summarise_at(c("duration", "fare", "transfers"),
               list(avg = ~.x[order(duration)[1:2]] %>% mean())) %>% 
  rename(grid.id = fromID) %>% 
  left_join(yamanote_area_grid) %>%
  st_as_sf(crs = 6677) %>% 
  st_join(hotels_sf, .)
